...
1. B) S3 Lifecycle Management - Automates storage class transitions
2. B) Model parallelism - Splits model across instances
3. B) Orchestrating ML workflows - Primary purpose of SageMaker Pipelines
4. B) Few false positives, many false negatives - High precision, low recall
5. B) Class imbalance in datasets - SMOTE creates synthetic minority samples
6. B) Amazon MWAA (Managed Workflows for Apache Airflow) - Managed Airflow service
7. C) Bayesian optimization - Most efficient search strategy
8. B) Concurrent access from multiple instances - EFS advantage over S3
9. B) Elastic Weight Consolidation (EWC) - Prevents catastrophic forgetting
10. B) Visual data preparation without coding - Data Wrangler's main benefit
11. C) Canary deployment - Tests with small traffic percentage
12. B) Number of shards - Determines Kinesis throughput capacity
13. A) L1 (Lasso) regularization - Drives weights to zero for feature selection
14. B) Training and batch inference - Offline store purpose
15. B) Amazon Macie - Detects PII in datasets
16. C) Repeating patterns at regular intervals - Definition of seasonality
17. B) Factorization Machines - Designed for recommendation systems
18. B) Prevent exploding gradients - Purpose of gradient clipping
19. C) Amazon Athena - Serverless S3 querying
20. C) Model's ability to discriminate between classes - AUC measures discrimination
21. B) Mode imputation or creating a 'missing' category - Handle categorical missing values
22. B) Pay-per-request pricing with automatic scaling - Serverless inference advantage
23. D) All of the above - All help with ML experiment management
24. B) Using pre-trained models as starting points - Transfer learning definition
25. C) Root Mean Squared Error (RMSE) - Appropriate for regression
26. B) Establish normal data distribution patterns - Baseline job purpose
27. B) Label encoding - Preserves ordinal relationships
28. C) Cost savings up to 90% - Main Spot Instance benefit
29. B) Training jobs create models, processing jobs transform data - Key difference
30. C) AWS X-Ray - Provides distributed tracing
31. B) Combining multiple models to improve performance - Ensemble learning purpose
32. C) S3 Glacier Deep Archive - Lowest cost for long-term archival
33. B) Version and manage model artifacts - Model Registry purpose
34. B) Pruning and setting minimum samples per leaf - Reduces overfitting in trees
35. B) Event-driven automation and integration - EventBridge purpose
36. B) Amazon Transcribe - Speech-to-text service
37. C) Capture non-linear relationships - Polynomial features purpose
38. B) Multi-Model Endpoints - Hosts multiple models on single endpoint
39. B) Fully managed delivery to destinations - Firehose advantage over Streams
40. B) Gradients become very small in early layers during backpropagation - Vanishing gradient problem
41. B) Amazon Comprehend - Managed NLP service
42. B) Assess model performance and reduce overfitting bias - Cross-validation purpose
43. D) SageMaker doesn't support automatic scaling of training jobs - Training jobs don't auto-scale
44. A) Constant mean and variance over time - Stationarity definition
45. B) Target encoding or embedding - Effective for high-cardinality features
46. B) Scalable data preprocessing and feature engineering - Processing jobs benefit
47. D) All of the above - All help optimize costs
48. B) Normalize input batches to stabilize training - Batch normalization purpose
49. B) Blue/Green deployment - Fastest rollback capability
50. B) Bias monitoring and explainability - Clarify's production purpose