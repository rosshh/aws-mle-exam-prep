...
1. B) Apply SMOTE to balance the classes - Addresses class imbalance in fraud detection
2. A) Kinesis Data Streams with Lambda - Handles variable throughput with minimal overhead
3. B) Implement Model Monitor to detect drift and trigger retraining - Addresses seasonal changes
4. B) Configure early stopping with patience=5 - Prevents overfitting when validation loss increases
5. B) SageMaker Clarify for bias detection and monitoring - Purpose-built for bias monitoring
6. B) SageMaker serverless inference - Scales to zero and handles traffic spikes automatically
7. B) Glue DataBrew + SageMaker Feature Store - Cost-effective with schema evolution support
8. D) Covariate shift - Feature distributions changed, need to address input distribution changes
9. A) SageMaker + CloudTrail + Clarify + KMS - Addresses privacy, audit, explainability needs
10. D) All approaches combined - Comprehensive solution for large model training optimization
11. B) SageMaker endpoints with production variants - Enables A/B testing with traffic splitting
12. C) SageMaker DeepAR with related features - Best for time series with seasonality and external factors
13. D) All of the above components - Comprehensive CI/CD validation for ML models
14. B) Concept drift - New terminology represents concept changes, need recent labeled data
15. B) Amazon Rekognition Custom Labels - Minimal ML expertise required, scalable solution
16. B) Target encoding with cross-validation - Effective for high-cardinality features, prevents overfitting
17. A) SageMaker training jobs with spot instances - Maintains workflow while optimizing costs
18. C) Temporal distribution shift - Performance gap indicates data distribution changes over time
19. B) Deploy interpretable model with built-in explanations - Meets regulatory and latency requirements
20. B) Kinesis → Feature Store → SageMaker endpoints - Real-time personalization architecture
21. A) Data augmentation, dropout, transfer learning - Comprehensive overfitting prevention for small datasets
22. B) Lambda with batch size 1000 and appropriate timeout - Efficient batching for downstream processing
23. A) Multi-Model Endpoints with dynamic loading - Serves multiple models from single endpoint efficiently
24. B) S3 → EventBridge → Pipelines → Model Registry → CodePipeline - Complete automated ML pipeline
25. C) Kinesis → Lambda → DynamoDB lookup - Meets ultra-low latency requirements
26. D) Seasonal decomposition with trend-aware interpolation - Addresses systematic missing data patterns
27. A) VPC with private subnets → SageMaker in VPC → encrypted S3 - Meets HIPAA compliance requirements
28. D) All techniques together - Comprehensive model compression approach
29. C) Bayesian optimization with early stopping - Most efficient for limited budget
30. B) Amazon Comprehend's sentiment analysis - Minimal ML expertise, good out-of-box performance
31. A) Model Registry + Model Monitor + CloudTrail - Comprehensive model risk management
32. D) Amazon Personalize - Fully managed, scalable recommendation service
33. A) SageMaker Processing for unified feature engineering - Best practice for multi-modal data
34. B) Optimize for recall - Business cost justifies minimizing missed failures
35. C) Region-specific models with centralized training - Handles compliance and consistency requirements
36. D) Analyze importance and remove less predictive features - Balanced approach to multicollinearity
37. A) Kinesis → Lambda → Comprehend → SNS - Real-time social media monitoring architecture
38. B) Hybrid collaborative and content-based filtering - Addresses cold start problem effectively
39. A) Studio + Pipelines + Model Registry + CodePipeline - Complete MLOps workflow
40. B) Pre-compute and cache high-latency features - Optimizes for real-time latency requirements
41. A) SageMaker Clarify with Model Registry - Automated feature importance tracking
42. D) Pre-compute embeddings with approximate nearest neighbor - Efficient for large-scale recommendations
43. C) Stratified sampling by environmental conditions - Most comprehensive validation for autonomous systems
44. C) Batch generate overnight with searchable database - Meets 24-hour requirement efficiently
45. D) Combination approach based on utilization - Comprehensive cost optimization strategy
46. B) Domain adversarial training - Best approach for cross-hospital domain adaptation
47. A) API Gateway → Lambda → Comprehend → SQS - Robust multilingual ticket routing
48. D) EMR with Spark and ElastiCache - Handles scale requirements for dynamic pricing
49. D) Transfer learning with domain adaptation - Most practical federated learning approach
50. A) SageMaker RL with custom environments - Purpose-built for complex RL optimization problems